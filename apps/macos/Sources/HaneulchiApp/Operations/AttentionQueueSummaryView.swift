                ...
                VStack(alignment: .leading, spacing: HaneulchiMetrics.Spacing.sm) {
                    ForEach(items) { item in
                        HStack(spacing: 0) {
                            ...
                            Rectangle()
                                .fill(item.targetRoute == .reviewQueue ? HaneulchiChrome.Gradient.primaryEnd : HaneulchiChrome.State.warningSolid)
                                .frame(width: 4)
                            ...
                            VStack(alignment: .leading, spacing: HaneulchiMetrics.Spacing.sm) {
                                HStack(alignment: .firstTextBaseline) {
                                    Text(item.targetRoute == .reviewQueue ? "REVIEW READY" : "WAITING INPUT")
                                        ...
                                        .foregroundStyle(item.targetRoute == .reviewQueue ? HaneulchiChrome.Gradient.primaryEnd : HaneulchiChrome.State.warning)
                                        .bold()
                                    Spacer()
                                    Text("Now")
                                        .font(HaneulchiTypography.compactMeta)
                                ...
                                HStack {
                                    Button {
                                        ...
                                        Text(item.targetRoute == .reviewQueue ? "LAUNCH REVIEW" : "RESOLVE")
                                            .frame(maxWidth: .infinity)
                                    }
                                    .buttonStyle(HaneulchiButtonStyle(variant: item.targetRoute == .reviewQueue ? .primary : .secondary))
                                }
                            }
